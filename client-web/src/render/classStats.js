...
export function getAbilityCd(ab, customMasteries = null) {
  ...
  const cdFactor = 1.0 / (1.0 + (cdPoints - 1) * (cdMult - 1.0));
...
export const ROSTER_STATS = {
  hp: [65, 65, 70, 70, 70, 70, 75, 90, 90, 95, 95, 100, 110, 160, 160],
  speed: [2.70, 3.23, 3.36, 3.42, 3.44, 3.45, 3.50, 3.54, 3.54, 3.55, 3.56, 3.56, 3.57, 3.58, 3.61],
  throwPower: [1.12, 1.14, 1.17, 1.22, 1.27, 1.27, 1.32, 1.34, 1.37, 1.41, 1.41, 1.45, 1.77, 1.82, 1.82],
...
export function computeStatWithMastery(statType, baseVal, masteryPoints = 0) {
  ...
  if (statType === 'stealRad') {
    ...
    const stepMult = cfgKey ? getCfgNum(cfgKey, statType === 'hp' ? 1.08 : 1.04) : 1.04;
    // Apply additive mastery multiplier from game.cfg (1 baseline point = 1.0, +1 point = +step, -1 point = -step)
    const mult = 1.0 + (points - 1) * (stepMult - 1.0);
