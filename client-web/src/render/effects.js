...
export function drawEffectIcons(ctx, game, camX, camY) {
  ...
  for (let i = 0; i < effects.length; i++) {
    ...
    const y = Math.floor(en.Y - 42 - camY); // above the health bar
    
    if (img) {
      // Draw the icon 1.5x size (24x24 instead of 16x16)
      ...
      ctx.beginPath();
      ctx.arc(x + 12, y + 12, 15, -Math.PI / 2, -Math.PI / 2 + angle);
      ctx.strokeStyle = isCooldown ? 'rgba(255, 127, 17, 0.85)' : 'rgba(45, 200, 120, 0.85)';
      ctx.lineWidth = 3;
      ctx.stroke();
      
      offsetMap[en.id] += 30; // advance offset for next effect icon
    }
  }
  ...
  if (game.players) {
    for (const t of game.players) {
      if (t.isBoosting && t.health > 0) {
        ...
        const y = Math.floor(t.Y - 42 - camY);
        if (img) {
          ctx.drawImage(img, x, y, 24, 24);
          offsetMap[t.id] += 30;
        }
