...
export function getEntityCollisionBounds(entity) {
    ...
    if (entity.entityClass === 'LaneMinion') {
        const r = gameState.game?.underControl?.type === 'GOALIE' ? 20 : 10;
        return { x: entity.X - r, y: entity.Y - r, w: r * 2, h: r * 2, cx: entity.X, cy: entity.Y };
    }
    const isGoalie = entity.type === 'GOALIE', isTitan = entity.entityClass === 'Titan' || entity.type !== undefined;
    const w = isGoalie ? 50 : isTitan ? Math.max(15, (entity.width || 70) - 50) : (entity.width || 70);
    const h = isGoalie ? 30 : isTitan ? Math.max(15, (entity.height || 70) - 18) : (entity.height || 70);
    const x = isGoalie ? entity.X + ((entity.width || 70) - w) / 2 : isTitan ? entity.X + 25 : entity.X;
    const y = isGoalie ? entity.Y + ((entity.height || 70) - h) / 2 : isTitan ? entity.Y + 9 : entity.Y;
...
export function isValidTarget(caster, entity, slot = 'E') {
    ...
    if (!allowsMinions && !isTitan) return false;
    if (type === 'SPIDER') return isTitan;
    return (type === 'SUPPORT' && slotKey === 'R') ? isSameTeam : !isSameTeam;
}
...
export function drawTargetingOverlay(ctx, game, camX, camY) {
    ...
    const corners = [[rx - 2, ry - 2, 1, 1], [rx + rw + 2, ry - 2, -1, 1], [rx - 2, ry + rh + 2, 1, -1], [rx + rw + 2, ry + rh + 2, -1, -1]];
    for (const [cx, cy, sx, sy] of corners) {
        ctx.beginPath();
        ctx.moveTo(cx, cy + sy * 8);
        ctx.lineTo(cx, cy);
