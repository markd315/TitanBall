...
function gameLoop(timestamp) {
  ...
  if ([GamePhase.INGAME, GamePhase.COUNTDOWN, GamePhase.SCORE_FREEZE, GamePhase.WAIT_FOR_GAME, GamePhase.TUTORIAL, GamePhase.TUTORIAL_START].includes(gameState.phase)) {
    recordUserActivity();
  }
  if (Date.now() - (window.lastUserActivity || Date.now()) > 4 * 60 * 60 * 1000) {
